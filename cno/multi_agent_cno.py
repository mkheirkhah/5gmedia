...
CNO_PARA_LOSS_RATE = 5 # weight used in reward function
...
def testing(epoch, nn_model, log_file):
    ...
    # run test script
    os.system('python rl_test_cno.py ' + nn_model)
...
def central_agent(net_params_queues, exp_queues):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_test', 'wb') as test_log_file:
        ...
        actor = a3c_cno.ActorNetwork(sess,
                                 state_dim=[S_INFO, S_LEN], action_dim=A_DIM,
                                 learning_rate=ACTOR_LR_RATE)
        critic = a3c_cno.CriticNetwork(sess,
                                   state_dim=[S_INFO, S_LEN],
                                   learning_rate=CRITIC_LR_RATE)

        summary_ops, summary_vars = a3c_cno.build_summaries()
...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    net_env = env_cno.Environment(all_cooked_time=all_cooked_time,
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        while True:  # experience video streaming forever
            ...
            # CNO - added loss_rate and actual_data_sent
            delay, sleep_time, buffer_size, rebuf, \
            video_chunk_size, next_video_chunk_sizes, \
            end_of_video, video_chunk_remain, \
            loss_rate, actual_data_sent = \
            ...
            # last chunk bit rate (number)
            state[0, -1] = VIDEO_BIT_RATE[bit_rate] / float(np.max(VIDEO_BIT_RATE))  # last quality
            # past available bw (array)
            state[1, -1] = float(actual_data_sent) / float(delay) / M_IN_K  # kilo byte / ms
            # loss rate (array)
            state[2, -1] = loss_rate  # 10 sec
            #*******************************************************************
            ...
            entropy_record.append(a3c_cno.compute_entropy(action_prob[0]))
