...
LOG_FILE = './results/cno_log_alt' # MKS
TEST_LOG_PATH = './results/log_test'
TEST_LOG_FOLDER = './test_results/'
Q_TABLE_PATH = None

LOSS_PENALTY = 100 # MKS
...
class Tabular_Q(object):
    def __init__(self):
        ...
        if Q_TABLE_PATH is not None:
            ...
            # initialize the q table
            for bw in np.linspace(BW_MIN, BW_MAX, (BW_MAX - BW_MIN) / D_BW + 1):
                for lr in np.linspace(LR_MIN, LR_MAX, (LR_MAX - LR_MIN) / D_LR + 1):
                    for br in xrange(BR_LV):
                        #for c in xrange(N_CHUNK):
                        for a in xrange(BR_LV):
                            self.q_table[(bw, lr, br, a)] = 0.0
...
def testing(tabular_q, epoch):
    ...
    test_net_env = fixed_env_cno_alt.Environment(
    ...
    while True:  # serve video forever
        ...
        lr = min (int(loss_rate*100 / D_LR) * D_LR, LR_MAX)
        br = bit_rate
...
def main():
    ...
    net_env = env_cno_alt.Environment(all_cooked_time=all_cooked_time,
                              all_cooked_bw=all_cooked_bw)
    ...
    video_counter = 0 # MKS
    with open(LOG_FILE + '_tabular', 'wb') as lf:
        while True:
            delay, sleep_time, buffer_size, rebuf, \
                video_chunk_size, next_video_chunk_sizes, \
                end_of_video, video_chunk_remain, \
